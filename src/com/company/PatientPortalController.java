...
public class PatientPortalController {
    ...
    public PatientPortalController(int userID, FXMLLoader loader) {
        ...
        try{
            ...
            ResultSet rs=st.executeQuery("SELECT Username,Password FROM PatientData WHERE PatientID='"+userID+"'");
            while(rs.next())
            {
    ...
    public void initialize() {
        ...
        try {
        	...
        	ResultSet rt=st.executeQuery("select concat(firstname,concat(' ',lastname)) from employee WHERE type=0");
        	while(rt.next())
        	{
        		data.add(rt.getString("concat(firstname,concat(' ',lastname))"));
        	}
        ...
        ObservableList<String> PatientDoctorList = FXCollections.observableArrayList(data);
    ...
    protected void onUpdateInfoClick(ActionEvent event) throws IOException {
        ...
        if (Objects.equals(FirstNameField.getText(), "") ||
            ...
            try {
            	...
            	Connection connect=conn.getdbconnection();
            	Statement st=connect.createStatement();
            	st.executeUpdate("UPDATE PatientData SET FirstName='"+FirstNameField.getText()+"',LastName='"+LastNameField.getText()+"',DOB='"+DOBField.getValue().toString()+"',Pharmacy='"+PharmacyField.getText()+"',PhoneNo='"+Integer.parseInt(PhoneField.getText())+"',Address='"+AddressField.getText()+"',Insurance='"+InsuranceField.getText()+"' WHERE PatientID='"+userID+"' ");
            }
            catch(Exception e)
            {
    ...
    protected void onUpdateAccountClick(ActionEvent event) {
        if (OldUsernameField.getText().equals("") ||
            ...
            try {
                ...
                Connection connect=conn.getdbconnection();
                Statement st=connect.createStatement();
            	ResultSet rt=st.executeQuery("SELECT Username from PatientData where Username='"+NewUsernameField.getText()+"'");
            	if(rt.next())
            	{
            		temp=rt.getString("Username");
            	}
        ...
        } else if (NewUsernameField.getText().equals(temp)) {
            UpdateAccountStatusLabel.setTextFill(Color.RED);
            ...
            try {
            	...
            	Connection connect=conn.getdbconnection();
            	Statement st=connect.createStatement();
            	st.executeUpdate("UPDATE PatientData SET Username='"+NewUsernameField.getText()+"',Password='"+NewPasswordField.getText()+"' WHERE PatientID='"+userID+"'");
            }
            catch(Exception e)
            {
    ...
    protected void onNewUsernameFieldType(Event event) {
        ...
        try {
        	...
        	Connection connect=conn.getdbconnection();
        	Statement st=connect.createStatement();
        	ResultSet rt=st.executeQuery("SELECT * from PatientData WHERE Username='"+NewUsernameField.getText()+"'");
        	if(rt.next())
        	{
        		NewUsernameStatusLabel.setTextFill(Color.RED);
    ...
    protected void onChangeDoctorClick(ActionEvent event) {
        if (verifyDoctorChangeField.getText().equals(password)) {
            if (ChangeDoctorList.getSelectionModel().getSelectedItem() == null) {
                ...
                try {
                	String[] name = ChangeDoctorList.getSelectionModel().getSelectedItem().split(" ");
                	System.out.println(name[0]+name[1]);
                	connection c=new connection();
                	Connection conn=c.getdbconnection();
                	Statement st=conn.createStatement();
                	ResultSet rt=st.executeQuery("SELECT UserID from Employee WHERE FirstName='"+ name[0]+"' and LastName='"+name[1]+"'");
                	if(rt.next())
                	{
                	st.executeUpdate("UPDATE PatientData SET DoctorID="+rt.getInt("userID")+" WHERE PatientID="+userID+"");	
                	}
